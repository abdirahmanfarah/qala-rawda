    ...
    <div className="hall">
      ...
      <div className="daily-widgets">
        ...
        <div className="daily-card" onClick={() => router.push('/calendar')}>
          {hijriData ? (
            <>
              <div>{hijriData.hijri?.date} {hijriData.hijri?.month?.ar} {hijriData.hijri?.year}</div>
              <div>{hijriData.hijri?.day} {hijriData.hijri?.month?.en} {hijriData.hijri?.year}</div>
              <div>{hijriData.gregorian?.date}</div>
            </>
          ) : (
            <div>Loading...</div>
          )}
        ...
        <div className="daily-card" onClick={() => dua && setDuaModalOpen(true)}>
          {dua ? (
            <>
              <div>{dua.arabic ? dua.arabic.slice(0, 80) + (dua.arabic.length > 80 ? '...' : '') : ''}</div>
              <div>{dua.title || (dua.translation ? dua.translation.slice(0, 80) + '...' : '')}</div>
            </>
          ) : (
            <div>Loading...</div>
          )}
        ...
        <div className="daily-card" onClick={() => hadith && setHadithModalOpen(true)}>
          {hadith ? (
            <>
              <div>{hadith.arabic ? hadith.arabic.slice(0, 80) + (hadith.arabic.length > 80 ? '...' : '') : ''}</div>
              <div>{hadith.english ? hadith.english.slice(0, 80) + (hadith.english.length > 80 ? '...' : '') : ''}</div>
            </>
          ) : (
            <div>Loading...</div>
          )}
